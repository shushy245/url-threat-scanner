...
- **A URL scanner is an SSRF engine unless you stop it from being one.** The guard rejects
  ...
  including obfuscated encodings (`2130706433`, `0x7f.0.0.1`, `127.1`) and IPv4-mapped IPv6 — then
  re-checks **after DNS resolution** and on **every redirect hop**, because a guard that only reads
  the string is beaten by a public hostname resolving to `127.0.0.1`.
- **Submitted URLs carry session tokens.** Logs keep query keys and drop values. Logging one verbatim
  would turn our logs into a credential store.
...
(the atomic publish log) · `dlq_event` (where a failed message *would* be saved for manual replay).
...
`unknown` — every check errored — is a distinct verdict from `clean`, because a security product must
never say "safe" when it means "couldn't tell".
...
of `scan_check` rows. Checks take an injected `Clock` and `Random`, so "randomized results" doesn't
mean a flaky suite (ADR-0013).
...
- **No cache for popular URLs.** Every submission is its own audit record (ADR-0003), so the fix is a
  short-TTL cache *below* the check layer, invisible to the API.
- **No list endpoint.** Ids are sortable and keyset pagination is designed for; nothing depends on it.
...
Every variable is validated with Zod at startup (`src/config.ts`), so a process refuses to boot on a
bad value instead of failing at the first request. The full list lives there; the ones worth knowing:

| Variable | Default | Purpose |
|---|---|---|
| `DATABASE_URL` · `RABBITMQ_URL` | — | Required connection strings |
| `CHECK_MODE` | `simulated` | `real` refuses to start until the adapters exist |
...
`.env.docker` is committed on purpose — only local credentials matching Compose, which is what makes
one command enough.
...
src/
  ...
  domain/scan/  model, selectors, translator
  repositories/ port + Drizzle implementation; the outbox write lives here
  events/       versioned event schemas · db/  schema, client, migrations
  utils/        id generation, SSRF guard, URL normalization, logger
  main.api.ts · main.relay.ts · main.worker.ts    composition roots
docs/adr/       13 decision records, each a 20-second read
```

Every dependency is built in one place per process — the `main.*.ts` composition root — which is what
lets a test swap in a fake repository and exercise identical wiring.

## Design decisions

All thirteen are in [`docs/adr/`](docs/adr/): issue, decision, trade-offs, one-sentence summary.
Start with [0001](docs/adr/adr-0001-rabbitmq-transactional-outbox.md) (the outbox, reversing an
earlier choice), [0003](docs/adr/adr-0003-idempotency-key.md) (idempotency keys over URL caching),
[0004](docs/adr/adr-0004-security-posture.md) (SSRF guard; no rate limiter on purpose) and
[0011](docs/adr/adr-0011-drain-holds-locks-across-publish.md) (locks held across the publish).
